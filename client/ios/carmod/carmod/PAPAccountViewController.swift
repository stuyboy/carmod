...
class PAPAccountViewController: PAPPhotoTimelineViewController {
  ...
  override func viewDidLoad() {
    ...
    let PICTURE_SIZE: CGFloat = 100.0
    let profilePictureBackgroundView = UIView(frame: CGRectMake(self.view.frame.width/2-PICTURE_SIZE/2, 38.0, PICTURE_SIZE, PICTURE_SIZE))
    ...
    layer.cornerRadius = PICTURE_SIZE/2
    layer.masksToBounds = true
    self.headerView!.addSubview(profilePictureBackgroundView)
    
    let profilePictureImageView: PFImageView = PFImageView(frame: CGRectMake(self.view.frame.width/2-PICTURE_SIZE/2, 38.0, PICTURE_SIZE, PICTURE_SIZE))
    ...
    let PHOTO_COUNT_WIDTH: CGFloat = 45.0
    let PHOTO_COUNT_HEIGHT: CGFloat = 37.0
    let photoCountIconImageView: UIImageView = UIImageView(image: nil)
    photoCountIconImageView.image = UIImage(named: "IconPics.png")
    photoCountIconImageView.frame = CGRect(x: profilePictureImageView.frame.origin.x/2-PHOTO_COUNT_WIDTH/2, y: 50.0, width: PHOTO_COUNT_WIDTH, height: PHOTO_COUNT_HEIGHT)
    self.headerView!.addSubview(photoCountIconImageView)
    
    let photoCountLabel = UILabel(frame: CGRectMake(0.0, 94.0, profilePictureImageView.frame.origin.x, 22.0))
    ...
    let FOLLOWERS_HEIGHT: CGFloat = 37.0
    let followersIconImageView = UIImageView(image: nil)
    followersIconImageView.image = UIImage(named: "IconFollowers.png")
    let xPos: CGFloat = profilePictureImageView.frame.maxX+(self.view.frame.width-profilePictureImageView.frame.maxX)/2-FOLLOWERS_WIDTH/2
    followersIconImageView.frame = CGRect(x: xPos, y: 50.0, width: FOLLOWERS_WIDTH, height: FOLLOWERS_HEIGHT)
    self.headerView!.addSubview(followersIconImageView)
    
    let LABEL_WIDTH: CGFloat = self.view.frame.width-profilePictureImageView.frame.maxX
    let followerCountLabel = UILabel(frame: CGRectMake(profilePictureImageView.frame.maxX, 94.0, LABEL_WIDTH, 16.0))
