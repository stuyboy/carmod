...
class PAPUtility {
      ...
      likeActivity.saveInBackgroundWithBlock { (succeeded, error) in
        ...
        query.findObjectsInBackgroundWithBlock { (objects, error) in
          if error == nil {
            ...
            PAPCache.sharedCache.setAttributesForPhoto(photo, likers: likers, commenters: commenters, likedByCurrentUser: isLikedByCurrentUser, annotations: annotations)
          }
  ...
  class func queryForAnnotationsOnPhoto(photo: PFObject, cachePolicy: PFCachePolicy) -> PFQuery {
    let query: PFQuery = PFQuery(className: kAnnotationClassKey)
    query.whereKey(kAnnotationPhotoKey, equalTo: photo)
